...
from app.domains.emergencias.repository import obtener_detalle_atencion, obtener_informacion_basica
from app.core.schemas import AuditoriaRequest
from app.domains.emergencias.schemas import AuditoriaEmergenciasResponse

logger = logging.getLogger(__name__)

PROMPT_SISTEMA = """
Eres un experto auditor médico especializado en medicina de emergencia.
Tu tarea es evaluar si la atención de emergencia proporcionada cumple con guías clínicas
...
⚠️ CONTEXTO DE EMERGENCIAS:
- Este servicio atiende casos de alta complejidad y criticidad
- Los tiempos de respuesta deben ser estrictos según protocolos
- Se evalúa adherencia a guías de reanimación, trauma, IAM, ACV, etc.
...
- ¿Se prescribieron medicamentos ambulatorios necesarios (ej: EpiPen en anafilaxia)?
...
- NO penalices "tiempo insuficiente en emergencia" si hubo decisión de internación
- Ejemplo: Guías recomiendan 6 horas de observación → 2h en emergencia + internación = CUMPLE
- La internación es una decisión CORRECTA para observación prolongada
- Solo evalúa el tiempo en emergencia si el paciente fue dado de ALTA a domicilio directamente
...
def _construir_prompt_usuario(
    ...
    return f"""
Analiza la siguiente atención de emergencia y auditala según guías médicas internacionales.
...
2. Evalúa el ACTO MÉDICO CLÍNICO:
   ...
   - Tiempos: ¿Cumplieron tiempos recomendados?
...
4. Ejemplos de lo que NO debes evaluar:
   ...
   ❌ "Tiempo de observación insuficiente en emergencia" → Si muestra "INDICA INTERNACIÓN"
...
def formatear_historial(detalle: Dict) -> str:
    ...
    texto = f"""
=================================================================================
ATENCIÓN DE EMERGENCIA - DETALLE COMPLETO
=================================================================================
...
def auditar_emergencia(data: AuditoriaRequest) -> AuditoriaEmergenciasResponse:
    """Ejecuta la auditoría completa de un registro individual de emergencia."""
    ...
    if not info:
        raise ValueError(
            f"No se encontró la cuenta {gestion}/{internacion} en emergencias (sector 3). "
    ...
    diagnostico = "Diagnóstico de emergencia - Ver evoluciones clínicas"
    ...
    return AuditoriaEmergenciasResponse(**resultado_llm)
